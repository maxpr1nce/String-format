# Переменные
team1_num = 5
team2_num = 6
score_1 = 40
score_2 = 42
team1_time = 18015.2
team2_time = 20000.0

# Использование %
team1_message = "В команде Мастера кода участников: %d !" % team1_num
teams_message = "Итого сегодня в командах участников: %d и %d !" % (team1_num, team2_num)

# Использование format()
score_2_message = "Команда Волшебники данных решила задач: {} !".format(score_2)
time_message = "Волшебники данных решили задачи за {} с !".format(team2_time)

# Использование f-строк
teams_solved_message = f"Команды решили {score_1} и {score_2} задач."
challenge_result = ""

if score_1 > score_2 or (score_1 == score_2 and team1_time > team2_time):
    challenge_result = 'Победа команды Мастера кода!'
elif score_1 < score_2 or (score_1 == score_2 and team1_time < team2_time):
    challenge_result = 'Победа команды Волшебники Данных!'
else:
    challenge_result = 'Ничья!'

challenge_message = f"Результат битвы: {challenge_result}"

# Вычисление задач и среднего времени
tasks_total = score_1 + score_2
time_avg = (team1_time + team2_time) / 2
tasks_message = f"Сегодня было решено {tasks_total} задач, в среднем по {time_avg:.1f} секунды на задачу!"

# Печать результатов
print(team1_message)
print(teams_message)
print(score_2_message)
print(time_message)
print(teams_solved_message)
print(challenge_message)
print(tasks_message)